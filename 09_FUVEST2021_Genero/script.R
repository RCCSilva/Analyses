...
curso <- pdftools::pdf_text('fuvest2021_relacao_candidato_vaga.pdf')
...
curso_sen <- str_split(curso, "\\n") %>% 
 ...
 curso_df <- tibble(curso_id    = str_sub(curso_sen,1,3) %>% as.integer(),
                    curso_nome  = str_trim(str_remove(str_remove(curso_sen, '\\d{3}'), '-')))
...
curso_df <- curso_df %>% 
  mutate(curso_nome = case_when(curso_id == 100 ~ 'Administração, Economia, etc. - Ribeirão Preto',
                                ...
                                curso_id == 775 ~ 'Engenharia Mecânica, etc. - São Carlos',
...
convocados <- pdftools::pdf_text('fuvest_2021_chamada_1.pdf')
...
prop_fem_df <- convocados_df %>% 
  ...
  mutate(prop_fem = if_else(is.na(prop_fem), 0.0, prop_fem))
...
row_cursos <- as.integer(seq(1,nrow(cursos_ordered), length.out = 5))
...
for(i in seq_along(row_cursos)[-length(row_cursos)]){
  ...
  convocados_df %>% 
    ...
    labs(title = "Distribuição de gênero nos cursos da USP",
         subtitle = paste0('Cursos de ', cursos_ordered$n[i1], ' a ', cursos_ordered$n[i0], ' ingressantes'),
         x     = "",
         y     = 'Proporção',
         fill  = 'Classificação do Nome',
         caption = 'Fonte: Lista de 1ª chamada da FUVEST 2021
                    Elaboração: Rafael Coelho')
  
  ggsave(paste0('grafico',i,'.png'), width = 12, height = 7, units = 'in' )
}
